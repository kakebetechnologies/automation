...
$stmt->execute([':id' => $id]);
$result['trackingEvents'] = array_map(function ($e) {
  return [
...
$dnStmt = $pdo->prepare('SELECT pickup_location FROM dispatch_notes WHERE request_id = ?');
$dnStmt->execute([$id]);
$result['pickupLocation'] = $dnStmt->fetchColumn() ?: null;

json_ok(['request' => $result]);
